...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if roll == 1:
            ...
            num_total_dices += roll
    if rolled_once:
        return 1
    else:
        return num_total_dices
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for numroll in range(1, 11):
        currentavg = make_averaged(roll_dice, 1000)(numroll, dice)
        print (numroll, "dice scores", currentavg, "on average")
        if currentavg > numbiggest:
            numbiggest = currentavg
            biggest = numroll
    return biggest
...
def swap_strategy(score, opponent_score):
    ...
    5
    """
    locked = 1 + max(opponent_score % 10, opponent_score // 10)
    if (score + locked) * 2 == opponent_score:
        return 0
    elif score + locked == opponent_score * 2:
        return BASELINE_NUM_ROLLS
    else:
...
def final_strategy(score, opponent_score):
    ...
    ***Make use of the free_bacon rule more and more so that we may take
    less risks and whenever we see that we can become victims of hog_wild or
    swine_swap rules then we use free_bacon and save ourself from losing score
    or getting more chances of dice rolling 1.
    The strategy will first implement the get_a_mean strategy first because the probability of
    getting higher scores in the first or initial turns is more as compared to later turns and
    moreover in the later turns the risks would also be higher.
    This strategy would also decrease the risks of losing most marks if the player is
    nearer to winning and where it is most risky to lose.
    ***
    """
    get_a_mean = make_mean_strategy(4, num_rolls=5)
    using_comeback = make_comeback_strategy(8, num_rolls=5)
    if opponent_score//10+1+score >= goal :
        result = 0
    elif score >= 86 :
        result = 4
    elif opponent_score - score >= 22:
        result = get_a_mean(score, opponent_score)
        if result == 0:
            return 0
        result=result+3
